$(document).ready(function(){
	...
	// Show buy box
	$(".dash-contents .trade .trade-action .actions .buy").on("click", function(event){
	...
	// Show sell box
	$(".dash-contents .trade .trade-action .actions .sell").on("click", function(event){
		event.stopPropagation();
	...
	// Show forex box
	$(".dash-contents .calc-section .add-icon").on("click", function(event){
		event.stopPropagation();
		showSection(".forex-box");
	});

	// Hide forex box
	$(".forex-box span.close").on("click", function(event){
		event.stopPropagation();
		hideSection(".forex-box");
	});

	$(".dash-contents .trade .trade-action .trade-amount .trade-amount-add").on("click", function(event){
		event.stopPropagation();
		addAmount();
	});

	$(".dash-contents .trade .trade-action .trade-amount .trade-amount-minus").on("click", function(event){
		event.stopPropagation();
		minusAmount();
	});
	...
	$(".forex-box .second table tr").mouseleave(function(){
		var index = $(".forex-box .second table tr").index(this);
		var eq = index - 1;
		$(".instrument-box").hide();
	});

	$(".instrument-box").mouseenter(function(){
		var index = $(".instrument-box").index(this);
		var eq = index;
		$(".instrument-box").hide();
		$(".instrument-box").eq(eq).show();
	});
	...
	$(".forex-box .second table tr").on("click", function(){
		...
		if(numberOfInstrument < 4){
			...
			// Prepend instrument
			$(".dash-navbar .stock .dash-row").prepend("<div class=\"stock-box\"><span class=\"name\">Forex</span><span class=\"currency\">" + instrument + "</span><span class=\"close\"><svg class=\"lnr lnr-cross\"><use xlink:href=\"#lnr-cross\"></use></svg></span></div>");
		}
	...
	function orderBook(action){
		var orderBookIsVisible = $(".dash-contents .order-book-section").is(":visible");
		var calcIsVisible = $(".dash-contents .calc-section").is(":visible");
		var newsIsVisible = $(".dash-contents .news-section").is(":visible");
		var orderBookNotVisible = $(".dash-contents .order-book-section").is(":hidden");
		...
		if(action == "hide"){
			...
			$(".dash-contents .order-book-section").hide();
			$(".dash-contents .market").css("width", "74%");
			if((calcIsVisible) && (newsIsVisible)){
				$(".dash-contents .market").css({
					"width": "38%",
				});
